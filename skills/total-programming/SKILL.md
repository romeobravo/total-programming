...
Prefer choices that can be changed, replaced, or removed without rebuilding everything around them. A reversible decision keeps the next move yours. Require stronger evidence for commitments that are difficult to undo.

Momentum comes from acting on what you learn, not from refusing to change course. Preserve options through focused solutions and good boundaries, not speculative flexibility. The goal is not to anticipate every future change, but to keep the cost of responding to change low enough to keep moving.
...
Build a sequence of understandable changes toward the goal rather than one elaborate solution. Each step should deliver coherent value or answer a meaningful question, and build toward something larger.
...
Each principle has a one-word descriptor—Intent, Parsimony, Subtraction, Holism, Clarity, Atomicity, Falsification, Empiricism, Reversibility, Asymmetry, Progression—as shorthand for invoking it in design discussions and reviews.
